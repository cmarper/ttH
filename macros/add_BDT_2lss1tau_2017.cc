  ...
  */
  ...
  for(unsigned int i_tree=0;i_tree<treename.size();i_tree++){
    ...
    for(int i=0;i<nentries;i++){
      ...
      float mass_bjet2 = bjet2.M();
      LorentzVector bjet2_lv((*_recoPFJet_btag_pt)[1],(*_recoPFJet_btag_eta)[1],(*_recoPFJet_btag_phi)[1],mass_bjet2);
     
      if (_recoPFJet_untag_Wtag_px->size()==0) continue;
      ...
      nllKinFit = _nllKinFit;
      _pT_b_o_kinFit_pT_b = (bjet1.Pt()) / (_kinFit->fittedBJet().Pt());
      pT_b_o_kinFit_pT_b = _pT_b_o_kinFit_pT_b;
