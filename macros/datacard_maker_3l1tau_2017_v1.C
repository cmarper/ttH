  ...
  for(unsigned int i_cat=0;i_cat<cat_names.size();i_cat++){
    ...
    for(unsigned i_MC=0;i_MC<MC_sample_name.size();i_MC++){
      ...
      TH1F* h_MC_genmatch = single_plot(filename_MC[i_MC],
					...
					var,
					Form("MC_weight*PU_weight_ICHEP18*triggerSF_weight*leptonSF_ttH_weight*bTagSF_CSVshape_weight*event_weight_ttH*(1-0.1*recotauh_sel_isGenMatched[0])*(category==%i  && recotauh_sel_byLooseIsolationMVArun2017v2DBoldDMdR0p3wLT2017[0] &&",cat_fakes[i_cat]) + MC_sample_cut[i_MC] + " && " + cut_cat + ")",
					nbin,xmin,xmax);
