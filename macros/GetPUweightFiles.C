...
const map<string, string> PUSampleNameMap_ = {
...
void Set_up_PUWeights(const std::string& samplename)
{
    ...
    std::string fname_pu_mc = std::string("PUreweight/pileup_2017_v2.root");

	if (not PUSampleNameMap_.count(samplename)) {
		std::cerr << "sample name : " << samplename << " not exsit in the PU name map" << std::endl;
		assert(0);
	}
	
    LumiWeights_ = edm::LumiReWeighting(fname_pu_mc, fname_pu_data, PUSampleNameMap_.at(samplename), "pileup");
}

//void delete_PUWeight_hist();

float Get_PUweight(int nPU, bool isMC, const std::string& samplename)
{
        if (isMC)  Set_up_PUWeights(samplename);
