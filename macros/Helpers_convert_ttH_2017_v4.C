...
TMVA::Reader* BookLeptonMVAReader2017(std::string basePath, std::string weightFileName, std::string type)
{
   ...
   reader->AddVariable("LepGood_jetPtRelv2",                          &lepMVA_jetPtRelv2); //5
...
*/
    ...
    dir_out+="ttH/";
    list.push_back(dir_in+"syncNtuple_94X_v4.root");
  }
...
/*
  ...
  */
  ...
  vector<float> *_daughters_miniIsoChg_2;
  vector<float> *_daughters_miniIsoAll_2;
  vector<float> *_daughters_PFIsoChg_2;
  vector<float> *_daughters_PFIsoAll_2;
  vector<float> *_daughters_PFIsoAll04_2;
  ...
  tree->SetBranchAddress("daughters_miniIsoChg_2",&_daughters_miniIsoChg_2);
  tree->SetBranchAddress("daughters_miniIsoAll_2",&_daughters_miniIsoAll_2);
  tree->SetBranchAddress("daughters_PFIsoChg_2",&_daughters_PFIsoChg_2);
  tree->SetBranchAddress("daughters_PFIsoAll_2",&_daughters_PFIsoAll_2);
  tree->SetBranchAddress("daughters_PFIsoAll04_2",&_daughters_PFIsoAll04_2);
  ...
  vector<float> _recomu_miniIsoAll_2; 
  ...
  vector<float> _recoele_miniIsoAll_2;
  vector<float> _recoele_PFIsoChg_2;
  vector<float> _recoele_PFIsoAll_2;
  vector<float> _recoele_PFIsoAll04_2;
  ...
  tree_new->Branch("recomu_jetNDauChargedMVASel_2",&_recomu_jetNDauChargedMVASel_2);
  ...
  tree_new->Branch("recomu_miniIsoAll_2",&_recomu_miniIsoAll_2);
  ...
  tree_new->Branch("recoele_miniIsoAll_2",&_recoele_miniIsoAll_2);
  tree_new->Branch("recoele_PFIsoChg_2",&_recoele_PFIsoChg_2);
  tree_new->Branch("recoele_PFIsoAll_2",&_recoele_PFIsoAll_2);
  tree_new->Branch("recoele_PFIsoAll04_2",&_recoele_PFIsoAll04_2);
  ...
  for (int i=skip_entries;i<skip_entries+nentries;i++) {
    ...
    //if (_EventNumber == 15811778) cout << i << endl;
    //if (i==7816) cout << _EventNumber << " " << _RunNumber << " " << _lumi << endl; 
    //if (i==7817) cout << _EventNumber << " " << _RunNumber << " " << _lumi << endl;
    ...
    _recoele_miniIsoAll_2.clear();
    _recoele_PFIsoChg_2.clear();
    _recoele_PFIsoAll_2.clear();
    _recoele_PFIsoAll04_2.clear();
    ...
    _daughters_miniIsoChg_2 = 0;
    _daughters_miniIsoAll_2 = 0;
    _daughters_PFIsoChg_2 = 0;
    _daughters_PFIsoAll_2 = 0;
    _daughters_PFIsoAll04_2 = 0;
	...
	float miniRelIso = miniRelIsoCharged + miniRelIsoNeutral;
        float miniIsoChg_2 = (*_daughters_miniIsoChg_2)[i_daughter]; //rel
        float miniIsoAll_2 = (*_daughters_miniIsoAll_2)[i_daughter]; //rel
	float sip = (*_SIP)[i_daughter];
	bool looseID = (*_daughters_muonID)[i_daughter] & 1;
       
        if(daughter.Pt()>5 && fabs(daughter.Eta())<2.4 && fabs(dxy)<=0.05 && fabs(dz)<0.1 && miniIsoAll_2<0.4 && sip < 8 && looseID){
	  lepMVA_pt = daughter.Pt();
	  lepMVA_eta = daughter.Eta();
	  lepMVA_jetNDauChargedMVASel = (*_daughters_jetNDauChargedMVASel)[i_daughter]; //old for now
	  lepMVA_miniRelIsoCharged = miniIsoChg_2; //from nanoAOD
	  lepMVA_miniRelIsoNeutral = miniIsoAll_2 - miniIsoChg_2; //from nanoAOD
	  ...
	  lepMVA_jetBTagCSV = 0.;
	    ...
	    lepMVA_jetPtRelv2 = (*_daughters_jetPtRel)[i_daughter]; //old for now
	    lepMVA_jetPtRatio = std::min((*_daughters_jetPtRatio)[i_daughter],float(1.5)); //old for now     
	  ...
	  lepMVA_mvaId = (*_daughters_lepMVA_mvaId)[i_daughter]; //we dont use daughters_eleMVA_HZZ anymore
	  ...
	  if(leptonMVA<0.90) // new definition of cone pt from multileptons
    ...
    for(unsigned int i_mu=0; i_mu<reco_mus.size(); i_mu++){
      ...
      _recomu_miniIsoChg_2.push_back( muon.Pt()*(*_daughters_miniIsoChg_2)[i_daughter] ); //not the relative one
      _recomu_miniIsoAll_2.push_back( muon.Pt()*(*_daughters_miniIsoAll_2)[i_daughter] ); //not the relative one
	...
	_recomu_jetPtRel_2.push_back( (*_daughters_jetPtRel_2)[i_daughter] );
        ...
        _recomu_jetPtRatio_2.push_back( (*_daughters_jetPtRatio_2)[i_daughter] );
      }
      else{
	...
	_recomu_jetPtRel_2.push_back( 0. );
        ...
        _recomu_jetPtRatio_2.push_back( 1. );
      }
      ...
      lepMVA_miniRelIsoNeutral = (_recomu_miniIsoAll_2[i_mu] - _recomu_miniIsoChg_2[i_mu]) / lepMVA_pt;
      ...
      //Fakeable - to be updated for 2017!
      bool isfakeable=false;
	...
	_n_recomu_fakeable++;
      ...
      bool mediumID = ((*_daughters_muonID)[i_daughter]>>2) & 1; //HIP safe medium muon ID
	...
	float miniRelIso = miniRelIsoCharged + miniRelIsoNeutral;
	float sip = (*_SIP)[i_daughter];
        float miniIsoChg_2 = (*_daughters_miniIsoChg_2)[i_daughter]; //relative
        float miniIsoAll_2 = (*_daughters_miniIsoAll_2)[i_daughter]; //relative 
        float PFIsoAll04_2 = (*_daughters_PFIsoAll04_2)[i_daughter]; //relative
	...
	float eleMVA =  daughter.Pt()>10 ? eleMVAnt : eleMVA_HZZ;
        ...
        bool LooseIdWP = (*_daughters_iseleWPLoose)[i_daughter]; //FIXME: am I using this correctly?

	int eleMissingHits = (*_daughters_eleMissingHits)[i_daughter];
	
	if(daughter.Pt()>7 && fabs(daughter.Eta())<2.5 && fabs(dxy)<=0.05 && fabs(dz)<0.1 && miniIsoAll_2<0.4 && sip < 8 && LooseIdWP && eleMissingHits<=1){
	  ...
	  if(_daughters_dR_closest_jet[i_daughter]<0.4){   
	    ...
	    lepMVA_jetBTagCSV = std::max((*_daughters_jetBTagCSV)[i_daughter],float(0.)); 
            ...
            lepMVA_jetPtRelv2 = (*_daughters_jetPtRel)[i_daughter];
            lepMVA_jetPtRatio = 1 / ( 1 + PFIsoAll04_2 ); //new. PFIsoAll04_2 is relative
            lepMVA_jetBTagCSV = std::max((*_daughters_jetBTagCSV)[i_daughter],float(0.));
          }
    ...
    for(unsigned int i_ele=0; i_ele<reco_eles.size(); i_ele++){
      ...
      _recoele_miniIsoChg_2.push_back( elec.Pt()*(*_daughters_miniIsoChg_2)[i_daughter] );
      _recoele_miniIsoAll_2.push_back( elec.Pt()*(*_daughters_miniIsoAll_2)[i_daughter] );
      _recoele_PFIsoChg_2.push_back( elec.Pt()*(*_daughters_PFIsoChg_2)[i_daughter] );
      _recoele_PFIsoAll_2.push_back( elec.Pt()*(*_daughters_PFIsoAll_2)[i_daughter] );
      _recoele_PFIsoAll04_2.push_back( elec.Pt()*(*_daughters_PFIsoAll04_2)[i_daughter] );
	...
	_recoele_jetPtRatio.push_back( (*_daughters_jetPtRatio)[i_daughter] );
        _recoele_jetPtRatio_2.push_back( (*_daughters_jetPtRatio_2)[i_daughter] );
      }
      else{
	...
	_recoele_jetPtRatio.push_back( 1. );
        _recoele_jetPtRatio_2.push_back( 1. );
      }
      ...
      lepMVA_miniRelIsoCharged = _recomu_miniIsoChg_2[i_ele]/lepMVA_pt;
      lepMVA_miniRelIsoNeutral = (_recomu_miniIsoAll_2[i_ele] - _recomu_miniIsoChg_2[i_ele]) / lepMVA_pt;
      ...
      if(_recoele_leptonMVA[i_ele]<0.90)
